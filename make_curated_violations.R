...
d_health <-
  read_csv(fs::path("data-raw", "health_violations.csv"),
         col_types = cols_only(
  ...
  rename(
    ...
    date = ISSUED) |>
  mutate(violation_city_category = "health")
...
d_housing <-
  ...
  rename(
    ...
    violation_type = `VIOLATION TYPE`,
    date = ISSUED) |>
  mutate(violation_city_category = "housing")

d <-
  bind_rows(d_health, d_housing) |>
  mutate(across(c(violation_city_category), as.factor))
